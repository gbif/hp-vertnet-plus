...
Many folks in our community are looking to the Creative Commons to address the issue of licensing. The Creative Commons “is a nonprofit organization that enables the sharing and use of creativity and knowledge through free legal tools.”(1) Their tools are free, easy to use, well documented, and accepted in most countries around the world. The licenses they offer are standardized and meet three important criteria - they are all based firmly in legal standards, they are easy for humans to read and understand (most humans anyway), and they are machine-readable.
...
Commits the contents of the dataset to the public domain (where these facts live already);
Waives any and all rights to the data and dataset (which in almost all cases, never existed to begin with);
Gives users the freedom to share or use it however they see fit;
Exempts the rights holder (if there was one) from any liability for the ways in which the data or dataset are used; and,
Is already in use for data shared by a wide-range of scientific institutions.
...
“ODbL covers only database
Still a license needed for (copyrightable) content
Standard licenses that can be used for content: CC licenses or CC0 (or PDDL)
Scientific content = 99% facts = no copyright applicable => Use CC0 to clearly mark it as such
User happy”(4)
...
As with all VertNet resources, if you have any questions or comments, please let us know (norms for data use and publication).
...
http://sciencecommons.org/resources/faq/database-protocol/#why_not_sa
Personal Correspondence between David Bloom and Peter Desmet, 14 March 2014
Resources
